...
const AllProducts = () => {
  ...
  const handleUpdate = () => {
    ...
    axios
      .put(`http://${backendip}:3000/update-product/${editProductId}`, {
        name: editedName,
  ...
  const handleDelete = (productId) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        axios
          .delete(`http://${backendip}:3000/delete-products/${productId}`)
          .then((response) => {
            if (response.status === 200) {
              Swal.fire(
                "Deleted!",
            ...
            (number) => (
              <li key={number} className="cursor-pointer mx-1">
                <a
                  ...
                  className="block px-3 py-2 bg-gray-200 text-gray-700 hover:bg-gray-300 rounded"
                >
                  {number + 1}
                </a>
              </li>
